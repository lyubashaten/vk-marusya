...
export const useTopMoviesStore = defineStore('topMovies', {
  ...
  actions: {
    async fetchTopMovies() {
      ...
      this.error = null
      try {
        const response = await axios.get<Movie[]>('https://cinemaguide.skillbox.cc/movie/top10')
  ...
  getters: {
    movieByPosition: (state) => {
      return (position: number) => {
        if (position < 1 || position > state.movies.length) return null
